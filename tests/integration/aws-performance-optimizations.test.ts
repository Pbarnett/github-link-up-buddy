...
interface KMSDataKeyCache {
  getDataKey(keyId: string, kmsClient: KMSClient): Promise<{ dataKey: Uint8Array; plaintext: Uint8Array }>;
}

interface SecretCache {
  getSecret(secretId: string, secretsClient: SecretsManagerClient): Promise<any>;
}

interface MultiRegionClientManager {
  getClients(preferredRegion?: string): { kms: KMSClient; secrets: SecretsManagerClient };
  executeWithFailover<T>(operation: (clients: any) => Promise<T>): Promise<T>;
}
