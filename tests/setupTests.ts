...
// ==============================================================================
// REACT DAY PICKER MOCKS
// ==============================================================================

vi.mock('react-day-picker', () => ({
  DayPicker: ({ children, className, classNames, components, onSelect, selected, ...props }: any) => {
    // Create a simple calendar grid mock
    ...
    return createElement('div', {
      ...
      // Calendar header
      createElement('div', { key: 'header', className: 'day-picker-header' }, [
        components?.IconLeft && createElement('button', {
          key: 'prev',
          'aria-label': 'Previous month',
          onClick: () => {}
        }, createElement(components.IconLeft)),
        createElement('div', { key: 'month', className: 'month-label' }, 'January 2025'),
        components?.IconRight && createElement('button', {
          key: 'next', 
          'aria-label': 'Next month',
          onClick: () => {}
        }, createElement(components.IconRight))
      ]),
      // Calendar grid with clickable dates
      createElement('div', { key: 'grid', className: 'day-picker-grid' }, 
...
// Make useController globally available
;(globalThis as Record<string, unknown>).useController = vi.fn((options) => {
  const defaultValues = {
  ...
  return {
    field: {
      ...
      value: defaultValues[options?.name as keyof typeof defaultValues] || '',
      onChange: vi.fn(),
      onBlur: vi.fn()
    },
    fieldState: {
...
;(globalThis as Record<string, unknown>).Plane = createMockIcon('Plane')
