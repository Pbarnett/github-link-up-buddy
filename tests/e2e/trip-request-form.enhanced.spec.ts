...
test.describe('Enhanced Trip Request Form', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await testSteps.navigateAndWait(page, '/trip/new', 'form, [data-testid="trip-request-form"]');
  });

  test('form renders with proper accessibility', async ({ page, testInfo }) => {
    try {
      ...
      await testSteps.takeScreenshotOnFailure(page, testInfo);
      throw error;
    }
  });

  test('validates required fields with proper error messages', async ({ page, testInfo }) => {
    try {
  ...
  test('handles NYC airports selection properly', async ({ page, testInfo }) => {
    try {
  ...
  test('config-driven defaults load correctly', async ({ page, testInfo }) => {
    try {
  ...
  test('form accessibility compliance - WCAG 2.2 AA', async ({ page, testInfo }) => {
    try {
