-- Migration: Fix critical notification system schema issues
-- This fixes the failing production readiness tests
...
COMMENT ON TABLE notification_templates IS 'Notification templates for email, SMS, and other channels';
COMMENT ON COLUMN notification_templates.name IS 'Unique template identifier';
COMMENT ON COLUMN notification_templates.notification_type IS 'Type of notification (booking_success, price_alert, etc.)';
COMMENT ON COLUMN notification_templates.channel IS 'Delivery channel (email, sms, push, in_app)';
COMMENT ON COLUMN notification_templates.body_text IS 'Plain text template with {{variable}} placeholders';
COMMENT ON COLUMN notification_templates.body_html IS 'HTML template with {{variable}} placeholders (email only)';
...
COMMENT ON COLUMN notifications.booking_id IS 'Associated booking ID if applicable';
