...
CREATE INDEX IF NOT EXISTS idx_payment_method_audit_user_id ON payment_method_audit(user_id);
CREATE INDEX IF NOT EXISTS idx_payment_method_audit_payment_method_id ON payment_method_audit(payment_method_id);
CREATE INDEX IF NOT EXISTS idx_payment_method_audit_created_at ON payment_method_audit(created_at);
CREATE INDEX IF NOT EXISTS idx_payment_method_audit_action ON payment_method_audit(action);
...
DO $$
BEGIN
  -- Only add comments if tables/columns exist
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'payment_methods') THEN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'payment_methods' AND column_name = 'encrypted_payment_data') THEN
      COMMENT ON COLUMN payment_methods.encrypted_payment_data IS 'KMS encrypted JSON containing sensitive payment data';
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'payment_methods' AND column_name = 'encryption_key_id') THEN
      COMMENT ON COLUMN payment_methods.encryption_key_id IS 'AWS KMS key ID used for encryption';
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'payment_methods' AND column_name = 'stripe_payment_method_id') THEN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'payment_method_audit') THEN
