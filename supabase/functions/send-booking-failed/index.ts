...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    console.log("Processing booking failure notification for:", booking_request_id);
    ...
    if (existingNotification) {
      console.log("Failure email already sent for booking:", booking_request_id);
    ...
    const htmlTemplate = await Deno.readTextFile(
      new URL("../templates/booking-failed.html", import.meta.url)
    );

    const errorMessage = bookingRequest.error || "An unexpected error occurred during booking";
    const filledHtml = htmlTemplate.replace("{{errorMessage}}", errorMessage);

    // Send email via Resend
    await sendEmail({
      to: profile.email,
      subject: "❌ Flight Booking Could Not Be Completed",
      html: filledHtml,
    });
    ...
    await supabase.from("notifications").insert({
      ...
      booking_request_id,
      type: "failure",
      payload: { email_sent: true, recipient: profile.email, error: errorMessage }
    });

    console.log("Booking failure email sent successfully");
