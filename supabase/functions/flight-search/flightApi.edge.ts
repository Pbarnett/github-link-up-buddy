...
export async function searchOffers(
  ...
  const uniqueOffers = allOffers.filter((offer, index, self) => {
    const key = `${offer.itineraries[0]?.segments[0]?.departure?.at}-${offer.itineraries[1]?.segments[0]?.departure?.at}-${offer.price?.total}`;
    return index === self.findIndex(o => {
      const otherKey = `${o.itineraries[0]?.segments[0]?.departure?.at}-${o.itineraries[1]?.segments[0]?.departure?.at}-${o.price?.total}`;
      return key === otherKey;
    });
  });
  ...
  const durationFilteredOffers = uniqueOffers.filter((offer: any, i: number) => {
    const outbound = offer.itineraries?.[0]?.segments?.[0]?.departure?.at;
    const inbound = offer.itineraries?.[1]?.segments?.[0]?.departure?.at;
    
    if (!outbound || !inbound) {
      console.log(`[flight-search] Offer #${i}: missing departure times`);
      return false;
    }
    ...
    // Fixed: Complete duration filter with both lower and upper bounds
