 ...
 * 
 * Usage:
 *   npx ts-node scripts/diagnoseTripOffers.ts <trip-request-id>
 * 
...
import { diagnoseOfferPipeline } from '../src/services/tripOffersService';
...
interface DiagnosticResult {
  ...
  stack?: string;
  pipelineStages?: {
    existence?: { passed: boolean; count: number };
    rawData?: { count: number; sample: any | null };
    transformation?: { count: number };
    validation?: { 
      passed: number;
      failed: number;
      acceptRate: string;
