...
function transformV2ToLegacy(v2Offer: Record<string, unknown>): Offer {
  ...
  return {
    ...
    airline:
      extractIataCode(String(v2Offer.origin_iata || '')) +
      '-' +
      extractIataCode(String(v2Offer.destination_iata || '')), // Simplified airline representation
...
export async function fetchTripOffers(
  ...
  console.log(
    '[🔍 SERVICE] Fetching trip offers for tripRequestId:',
    tripRequestId
  );
  ...
  if (tripError) {
    console.warn(
      '[🔍 SERVICE] Could not fetch trip request details for filtering:',
      tripError.message
    );
  ...
  console.log(
    `[🔍 SERVICE] Trip request is ${isRoundTripRequest ? 'round-trip' : 'one-way'}`
  );
  ...
  if (!v2Error && v2Data && v2Data.length > 0) {
    console.log(
      `[🔍 SERVICE] Found ${v2Data.length} raw offers in flight_offers_v2 table`
    );
    rawOffers = v2Data;
    usingV2Table = true;
  } else {
    ...
    console.log(
      '[🔍 SERVICE] No V2 offers found, checking legacy flight_offers table...'
    );
    ...
    if (legacyError) {
      console.error('[🔍 SERVICE] Error fetching from both tables:', {
        v2Error,
        legacyError,
      });
  ...
  console.log(
    `[🔍 SERVICE] Retrieved ${rawOffers.length} raw offers from ${usingV2Table ? 'V2' : 'legacy'} table`
  );
  ...
  try {
    ...
    const filterContext = createFilterContext({
      ...
      destination:
        tripRequest.destination_iata || tripRequest.destination || '',
    ...
    if (usingV2Table) {
      ...
      normalizedOffers = rawOffers.map(v2Offer => {
        ...
        if (v2Offer.price_total) {
          ...
          v2Offer.price &&
          typeof v2Offer.price === 'object' &&
          'total' in v2Offer.price
        ) {
    ...
    console.log(
      `[🔍 SERVICE] Executing ${pipelineType} filtering pipeline with ${filterPipeline.getFilters().length} filters`
    );

    const pipelineResult = await filterPipeline.execute(
      normalizedOffers as any,
      filterContext
    );

    console.log(
      `[🔍 SERVICE] Filtering completed: ${rawOffers.length} → ${pipelineResult.filteredOffers.length} offers`
    );
    ...
    const transformedOffers = pipelineResult.filteredOffers.map(offer => {
      if (usingV2Table) {
        ...
        return originalOffer
          ? transformV2ToLegacy(originalOffer)
          : transformUnifiedToLegacy(offer as any);
      } else {
        // For legacy offers, find original and return as-is
        const originalOffer = rawOffers.find(raw => raw.id === offer.id);
        return (
          (originalOffer as Offer) || transformUnifiedToLegacy(offer as any)
        );
      }
    });

    console.log(
      `[🔍 SERVICE] Successfully processed ${transformedOffers.length} filtered offers`
    );
    ...
    if (transformedOffers.length > 0) {
      console.log(
        ...
        transformedOffers.slice(0, 2).map(o => ({
          id: o.id,
          price: o.price,
          airline: o.airline,
          nonstop: o.nonstop,
    ...
    if (usingV2Table) {
      ...
      return isRoundTripRequest
        ? fallbackOffers.filter(
            offer => offer.return_date && offer.return_date.trim() !== ''
          )
        : fallbackOffers;
    } else {
 ...
 */
function transformUnifiedToLegacy(
  unifiedOffer: Record<string, unknown>
): Offer {
  const parseDateTime = (isoString: string) => {
    if (!isoString) return { date: '', time: '' };
    try {
  ...
  const segments = Array.isArray(unifiedOffer.segments)
    ? unifiedOffer.segments
    : [];
  ...
  const departure = parseDateTime(
    String((firstSegment.departure as any)?.at || '')
  );
  const returnInfo = parseDateTime(
    String((secondSegment.departure as any)?.at || '')
  );
  ...
  return {
    ...
    flight_number: String(
      (firstSegment.marketingCarrier as any)?.flightNumber || 'N/A'
    ),
    departure_date: departure.date,
    departure_time: departure.time,
    return_date: returnInfo.date,
    ...
    carrier_code: String(
      (firstSegment.marketingCarrier as any)?.iataCode || ''
    ),
    origin_airport: String((firstSegment.origin as any)?.iataCode || ''),
    destination_airport: String(
      (firstSegment.destination as any)?.iataCode || ''
    ),
