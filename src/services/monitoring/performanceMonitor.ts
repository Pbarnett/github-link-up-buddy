...
class PerformanceMonitor {
  ...
  getStats(): PerformanceStats {
    ...
    const averageLatency = this.metrics.reduce((sum, m) => sum + m.duration, 0) / totalOperations;
  ...
  getOperationStats(operation: string): Partial<PerformanceStats> {
    ...
    const averageLatency = operationMetrics.reduce((sum, m) => sum + m.duration, 0) / totalOperations;
    const successRate = (successfulOperations / totalOperations) * 100;
  ...
  getPerformanceTrends(minutes: number = 60): {
    timeline: Array<{ timestamp: number; averageLatency: number; operationCount: number }>;
  ...
  getSummaryReport(): {
    ...
    topSlowOperations: Array<{ operation: string; avgLatency: number; count: number }>;
  } {
    ...
    if (stats.errorRate > 5) {
      recommendations.push('High error rate detected - review error logs and implement retry logic');
    }
...
export function usePerformanceMonitor() {
  ...
  return {
    stats,
    isLoading,
    getOperationStats: (operation: string) => performanceMonitor.getOperationStats(operation),
    getTrends: (minutes?: number) => performanceMonitor.getPerformanceTrends(minutes),
