...
export class DuffelPaymentService {
  ...
  constructor(options: Partial<PaymentOptions> = {}) {
    ...
    console.log(`[DuffelPayments] Initialized with Duffel: ${this.options.enableDuffelPayments}, Stripe: ${!!this.stripe}`)
  }
  ...
  async createDuffelPaymentIntent(params: {
    ...
    try {
      ...
      const paymentIntent = await this.duffel.paymentIntents.create(paymentData)
      
      console.log(`[DuffelPayments] Duffel payment intent created: ${paymentIntent.id}`)
      return paymentIntent

    } catch (error) {
      console.error('[DuffelPayments] Duffel payment intent creation failed:', error)
      throw new Error(`Duffel payment intent creation failed: ${error.message}`)
    }
  }
  ...
  async createStripePaymentIntent(params: {
    ...
    try {
      ...
      console.log(`[DuffelPayments] Stripe payment intent created: ${paymentIntent.id}`)
      return paymentIntent

    } catch (error) {
      console.error('[DuffelPayments] Stripe payment intent creation failed:', error)
  ...
  async createPaymentIntent(params: {
    ...
    if (provider === 'duffel' && this.options.enableDuffelPayments) {
      try {
        ...
        console.warn('[DuffelPayments] Duffel payment failed, trying fallback:', error)
    ...
    if (this.stripe && (provider === 'stripe' || this.options.fallbackToStripe)) {
      try {
   ...
   */
  async confirmPayment(paymentId: string, provider: 'duffel' | 'stripe'): Promise<PaymentResult> {
    if (provider === 'duffel') {
      try {
        ...
        console.error('[DuffelPayments] Duffel payment confirmation failed:', error)
        throw error
      }
    }
    ...
    if (provider === 'stripe' && this.stripe) {
      try {
        ...
        console.error('[DuffelPayments] Stripe payment confirmation failed:', error)
        throw error
      }
    }
  ...
  private async processDuffelRefund(params: {
    ...
    try {
      ...
      const refund = await this.duffel.paymentIntents.refund(params.originalPaymentId, {
        amount: params.amount,
   ...
   */
  async getPaymentStatus(paymentId: string, provider: 'duffel' | 'stripe'): Promise<{
    status: string
    amount: string
    currency: string
    metadata?: Record<string, any>
