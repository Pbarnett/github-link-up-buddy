    ...
    for (let attempt = 1; attempt <= (config?.attempts ?? 1); attempt++) {
      try {
        ...
        if (error instanceof ApiClientError) {
          if (error.statusCode && !this.isRetryableStatusCode(error.statusCode, config)) {
            break;
          }
        }
...
export class TypeSafeApiClient implements ApiClient {
  ...
  async request<T extends keyof ApiEndpoints>(
    ...
    try {
      ...
      const result = await RetryHandler.executeWithRetry(
        () => this.executeRequest<ApiResponseType<T>>(finalConfig, options?.body, requestId),
        finalConfig.retry
      );
  ...
  private getHttpMethod<T extends keyof ApiEndpoints>(endpoint: T): ApiMethodType<T> {
  ...
  private async executeRequest<T>(
    ...
    try {
      ...
      return await ResponseTransformer.transform<T>(response, requestId ?? 'unknown');
 ...
 */
export const createApiClient = (baseURL: string, options?: Partial<ApiClientConfig>): TypeSafeApiClient => {
...
export const createAuthenticatedApiClient = (
  ...
  (client as any)['config'].interceptors.request.push((config: ApiRequestConfig) => {
