...
class RateLimiter {
  ...
  isAllowed(operation: keyof typeof RATE_LIMITS): boolean {
    ...
    const validRequests = operationRequests.filter(time => now - time < windowMs)
...
export class DuffelServiceGuided {
  ...
  constructor() {
    ...
    console.log(`[DuffelService] Initialized in ${this.isLive ? 'LIVE' : 'TEST'} mode`)
  }
  ...
  async createOfferRequest(params: {
    ...
    try {
      console.log('[DuffelService] Creating offer request:', JSON.stringify(requestData, null, 2))
  ...
  async getOffers(offerRequestId: string, limit = 50): Promise<Offer[]> {
    ...
    try {
      console.log(`[DuffelService] Retrieving offers for request: ${offerRequestId}`)
      ...
      console.log(`[DuffelService] Retrieved ${validOffers.length}/${offers.data.length} valid offers`)
      return validOffers
  ...
  async getOffer(offerId: string): Promise<Offer | null> {
    ...
    try {
      ...
      if (!validation.valid) {
        console.warn(`[DuffelService] Offer ${offerId} expired (${validation.minutesLeft} minutes left)`)
        return null
      }
  ...
  async createOrder(params: {
    ...
    try {
      console.log(`[DuffelService] Creating order with idempotency key: ${params.idempotencyKey}`)
      ...
      console.log(`[DuffelService] Order created: ${order.id}, Status: ${order.booking_reference}`)
      return order
  ...
  async cancelOrder(orderId: string): Promise<{ success: boolean; refund?: any }> {
    ...
    try {
      ...
      console.error(`[DuffelService] Failed to cancel order ${orderId}:`, error)
  ...
  private async withRetry<T>(
    ...
    for (let attempt = 0; attempt <= RETRY_CONFIG.maxRetries; attempt++) {
      try {
        ...
        const delay = RETRY_CONFIG.backoffMs[attempt] || RETRY_CONFIG.backoffMs[RETRY_CONFIG.backoffMs.length - 1]
        console.log(`[DuffelService] Attempt ${attempt + 1} failed, retrying in ${delay}ms`)
  ...
  private handleError(error: any, defaultMessage: string): Error {
    ...
    const errorDetail = error?.errors?.[0]?.detail || error?.message || defaultMessage
    
    // Map to user-friendly message per Implementation Guide
    const userMessage = DUFFEL_ERROR_MESSAGES[errorType as keyof typeof DUFFEL_ERROR_MESSAGES] || 
