...
export function validateDuffelEnvironment(): ValidationResult {
  ...
  if (testToken && !isValidDuffelToken(testToken)) {
    errors.push(
      'DUFFEL_API_TOKEN_TEST has invalid format (should start with duffel_test_)'
    );
  }

  if (liveToken && !isValidDuffelToken(liveToken)) {
    errors.push(
      'DUFFEL_API_TOKEN_LIVE has invalid format (should start with duffel_live_)'
    );
  }
  ...
  if (liveEnabled && !liveToken) {
    errors.push(
      'DUFFEL_LIVE_ENABLED is true but DUFFEL_API_TOKEN_LIVE is not set'
    );
  }

  if (!liveEnabled && liveToken) {
    warnings.push(
      'DUFFEL_API_TOKEN_LIVE is set but DUFFEL_LIVE_ENABLED is not true'
    );
  }
  ...
  if (webhookSecret && webhookSecret.length < 32) {
    warnings.push(
      'DUFFEL_WEBHOOK_SECRET seems too short (should be at least 32 characters)'
    );
  }
