...
export const BUSINESS_RULE_MESSAGING = {
  PRICE_INCLUDES: "All prices include round-trip, non-stop flights with carry-on bag",
  
  FLIGHT_TYPE: "All flights are round-trip and non-stop",
  
  BAGGAGE_INCLUDED: "Carry-on bag fee is included in the price shown",
  
  PREMIUM_EXPERIENCE: "We specialize in premium, non-stop flights for your convenience",
...
export const validateBusinessRules = {
  ...
  tripDuration: (minDuration: number, maxDuration: number) => {
    if (minDuration < PARKER_FLIGHT_BUSINESS_RULES.MIN_TRIP_DURATION) {
      throw new Error(`Minimum trip duration cannot be less than ${PARKER_FLIGHT_BUSINESS_RULES.MIN_TRIP_DURATION} day(s)`);
    }
    if (maxDuration > PARKER_FLIGHT_BUSINESS_RULES.MAX_TRIP_DURATION) {
      throw new Error(`Maximum trip duration cannot exceed ${PARKER_FLIGHT_BUSINESS_RULES.MAX_TRIP_DURATION} days`);
    }
  ...
  budget: (budget: number) => {
    if (budget < PARKER_FLIGHT_BUSINESS_RULES.MIN_BUDGET) {
      throw new Error(`Budget cannot be less than $${PARKER_FLIGHT_BUSINESS_RULES.MIN_BUDGET}`);
    }
    if (budget > PARKER_FLIGHT_BUSINESS_RULES.MAX_BUDGET) {
      throw new Error(`Budget cannot exceed $${PARKER_FLIGHT_BUSINESS_RULES.MAX_BUDGET}`);
    }
  }
};
