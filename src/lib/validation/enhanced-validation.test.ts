...
describe('Enhanced Error Formatting', () => {
  ...
  it('should treeify errors into nested structure', () => {
    ...
    if (!result.success) {
      ...
      expect(tree.properties?.name?.errors).toContain('String must contain at least 2 character(s)');
      expect(tree.properties?.email?.errors).toContain('Invalid email');
      expect(tree.properties?.age?.errors).toContain('Number must be greater than or equal to 18');
      expect(tree.properties?.hobbies?.errors).toContain('Array must contain at least 1 element(s)');
...
describe('Enhanced Schema Features', () => {
  ...
  it('should validate with enhanced error formatting', () => {
    ...
    const result = validateFormValuesEnhanced(mockConfig, {
      name: 'A',
      email: 'invalid-email'
    }, {
      prettify: true,
