...
import { , AirlineFilter, getAirlineName, getAvailableAirlinesFromOffers, ../filters/AirlineFilter'; } from '../core/types';
  AirlineFilter,
  getAirlineName,
...
const createMockAmadeusOffer = (
  ...
  rawData: {
    ...
    itineraries: [
      {
        segments: [{ carrierCode: carrierCodes[0] }],
      },
    ],
  },
});
...
describe('AirlineFilter', () => {
  ...
  describe('Error handling', () => {
    it('should handle errors gracefully and return original offers', async () => {
      ...
      const originalMethod = (
        filter as unknown as { extractAirlineCodesFromOffer: unknown }
      ).extractAirlineCodesFromOffer;
      (
        filter as unknown as { extractAirlineCodesFromOffer: unknown }
      ).extractAirlineCodesFromOffer = vi.fn(() => {
      ...
      (
        filter as unknown as { extractAirlineCodesFromOffer: unknown }
      ).extractAirlineCodesFromOffer = originalMethod;
    });
  });
  ...
  describe('Validation', () => {
    ...
    it('should warn about too many airline selections', () => {
      const context = createMockFilterContext({
        preferredAirlines: new Array(15)
          .fill(0)
          .map(
            (_, i) => String.fromCharCode(65 + i) + String.fromCharCode(65 + i)
          ), // ['AA', 'BB', 'CC', ...]
      });
