...
vi.mock('@/components/profile/ProfileV2', () => {
  ...
  return {
    ProfileV2: () => React.createElement('div', { 'data-testid': 'profile-v2' }, 'Profile Form V2')
  }
});
...
vi.mock('@/components/NotificationPreferences', () => {
  const React = require('react');
  return {
    NotificationPreferences: () => React.createElement('div', { 'data-testid': 'notification-preferences' }, 'Notification Preferences')
  }
});
...
vi.mock('@/components/AuthGuard', () => {
  const React = require('react');
  return {
    default: ({ children }: { children: React.ReactNode }) => React.createElement('div', { 'data-testid': 'auth-guard' }, children)
  }
});
...
describe('Profile Completeness Integration', () => {
  it('renders ProfileCompletenessIndicator in enhanced profile', async () => {
    ...
    const profileCompletenessElements = screen.getAllByText('Profile Completeness');
  ...
  it('displays field categories correctly', async () => {
    ...
    const identityVerificationElements = screen.getAllByText('Identity Verification');
  ...
  it('handles field clicks correctly', async () => {
    ...
    const fullNameField = screen.getByText('Full Name').closest('[role="button"]');
    if (fullNameField) {
