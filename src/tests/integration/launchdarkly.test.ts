...
describe('LaunchDarkly Integration Tests', () => {
  ...
  describe('Client-side Feature Flags', () => {
    ...
    it('should get JSON feature flag values with fallback', async () => {
      ...
      await featureFlagClient.initialize(mockUser);
      const result = await featureFlagClient.getJSONFlag('uiConfig', defaultConfig);
  ...
  describe('Feature Flag Types', () => {
    it('should support boolean flags', async () => {
      const result = await featureFlagManager.getBooleanFlag('testBoolFlag', mockUser, true);
      expect(typeof result).toBe('boolean');
    });

    it('should support string flags', async () => {
      const result = await featureFlagManager.getStringFlag('testStringFlag', mockUser, 'default');
      expect(typeof result).toBe('string');
    });

    it('should support number flags', async () => {
      const result = await featureFlagManager.getNumberFlag('testNumberFlag', mockUser, 42);
