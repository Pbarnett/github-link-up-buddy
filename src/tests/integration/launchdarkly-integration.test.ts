...
describe('LaunchDarkly Integration Tests', () => {
  ...
  describe('Service Initialization', () => {
    ...
    it('should handle initialization timeout gracefully', async () => {
      const timeoutError = new Error(
        'LaunchDarkly initialization timeout after 10000ms'
      );
      mockClient.waitForInitialization.mockRejectedValue(timeoutError);

      const context = LaunchDarklyContextManager.createAnonymousContext();

      // The service handles errors gracefully and continues without crashing
      await expect(
        LaunchDarklyService.initializeClient(context)
      ).resolves.not.toThrow();

      // Should not be initialized after timeout
    ...
    it('should handle network connectivity issues', async () => {
      ...
      // Should record failure - check service is not initialized
      expect(LaunchDarklyService.isInitialized()).toBe(false);
    });
  });
  ...
  describe('Feature Flag Evaluation', () => {
    ...
    it('should handle circuit breaker activation', () => {
      ...
      // Should activate fallbacks after multiple failures
  ...
  describe('Context Management', () => {
    ...
    it('should sanitize sensitive information from context', () => {
      ...
      const sanitized =
        LaunchDarklyContextManager.sanitizeContext(unsafeContext);
  ...
  describe('Resilience Features', () => {
    ...
    it('should track performance metrics', async () => {
      ...
      expect(metrics.initializationTime).toBeGreaterThanOrEqual(0); // Handle very fast initializations potentially returning 0
      expect(metrics.successCount).toBeGreaterThanOrEqual(0); // Legacy mode might not increment success count
  ...
  describe('Known Feature Flags Integration', () => {
    ...
    it('should have proper fallback values for all known flags', () => {
      KNOWN_FLAGS.forEach(flagKey => {
        const fallbackValue =
          LaunchDarklyFallbackManager.getFallbackValue(flagKey);
  ...
  describe('Error Recovery', () => {
    it('should recover from temporary network issues', async () => {
      // Initial failure
      mockClient.waitForInitialization.mockRejectedValueOnce(
      ...
      // Simulate successful reconnection by retrying with success
      mockClient.waitForInitialization.mockResolvedValueOnce(undefined);
      await LaunchDarklyService.initializeClient(context);
