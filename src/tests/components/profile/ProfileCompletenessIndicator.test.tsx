...
vi.mock('@/components/profile/ProfileCompletenessIndicator', () => ({
  default: ({ completion, profile }: any) => {
    ...
    const tier = percentage >= 90 ? 'Verified Traveler' : 
                 percentage >= 70 ? 'Complete Profile' : 
                 percentage >= 40 ? 'Basic Profile' : 'Getting Started';
    
    const benefitsMessage = percentage >= 80 ? 'Great progress!' : 'Let\'s build your profile!';
    const benefitsSubMessage = percentage >= 80 ? 'You\'re almost there' : 'Complete a few more fields';
    
    return (
      <div data-testid="profile-completeness-indicator">
        <h3>Profile Completeness</h3>
        <div>{percentage}% Complete</div>
        <div data-testid="progress-bar" data-value={percentage}>{percentage}%</div>
        <div>{tier}</div>
        ...
        {/* Next action button */}
        {percentage < 100 && (
          <button>Complete Next Step</button>
        )}
      </div>
    );
  }
...
describe('ProfileCompletenessIndicator', () => {
  ...
  const createMockProfile = (overrides: Partial<TravelerProfile> = {}): TravelerProfile => ({
    id: 'test-profile-id',
  ...
  const createMockCompletion = (overrides: Partial<ProfileCompletion> = {}): ProfileCompletion => ({
  ...
  it('renders with basic profile and completion data', () => {
    ...
    render(<ProfileCompletenessIndicator profile={profile} completion={completion} />);

    expect(screen.getByText('Profile Completeness')).toBeInTheDocument();
    expect(screen.getByText('85% Complete')).toBeInTheDocument();
    expect(screen.getByTestId('progress-bar')).toHaveAttribute('data-value', '85');
  });
  ...
  it('displays the correct completion tier badge', () => {
    ...
    testCases.forEach(({ percentage, expectedTier }) => {
      const completion = createMockCompletion({ completion_percentage: percentage });
      const { unmount } = render(<ProfileCompletenessIndicator profile={profile} completion={completion} />);
  ...
  it('shows appropriate benefits message based on completion level', () => {
    ...
    const { unmount } = render(<ProfileCompletenessIndicator profile={profile} completion={highCompletion} />);
    ...
    render(<ProfileCompletenessIndicator profile={profile} completion={lowCompletion} />);
  ...
  it('handles field clicks and shows toast notifications', () => {
    ...
    const fullNameField = buttons.find(button => button.textContent?.includes('Full Name'));
  ...
  it('handles missing profile data gracefully', () => {
    ...
    render(<ProfileCompletenessIndicator profile={incompleteProfile} completion={completion} />);
  ...
  it('handles edge cases for completion percentage', () => {
    ...
    const { unmount } = render(<ProfileCompletenessIndicator profile={profile} completion={zeroCompletion} />);
    ...
    render(<ProfileCompletenessIndicator profile={profile} completion={fullCompletion} />);
  ...
  it('renders field categories in correct order', () => {
    ...
    const categories = screen.getAllByText(/Information|Details|Preferences|Verification/);
  ...
  it('handles profile verification status correctly', () => {
    ...
    render(<ProfileCompletenessIndicator profile={verifiedProfile} completion={completion} />);
