../lib/business-rules/ConfigLoader';
../lib/business-rules/schema';
...
export function BusinessRulesProvider({ children }: { children: ReactNode }) {
  ...
  const loadConfig = async () => {
    try {
      setLoading(true);
      setError(null);
      const environment = import React from 'react';
import.meta.env.VITE_ENVIRONMENT || 'development';
      const newConfig = await configLoader.loadConfig(environment);
      setConfig(newConfig);
    } catch (err) {
