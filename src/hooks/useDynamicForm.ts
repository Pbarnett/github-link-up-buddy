...
export const useDynamicForm = (options: UseDynamicFormOptions): UseDynamicFormReturn => {
  ...
  const submitForm = useCallback(async () => {
    if (!formConfig || !onSubmit) {
      console.warn('Cannot submit form: missing configuration or submit handler');
      return;
    }
    ...
    try {
      ...
      if (!validationResult.isValid) {
        throw new Error(`Form validation failed: ${Object.values(validationResult.errors).join(', ')}`);
      }
