...
interface AnalyticsEvent {
  type: 'form_view' | 'field_interaction' | 'field_error' | 'form_submit' | 'form_abandon';
  fieldId?: string;
  fieldType?: string;
  fieldValue?: string;
...
export const useFormAnalytics = ({ formConfig, sessionId, userId }: FormAnalyticsConfig) => {
  ...
  const trackFieldError = useCallback((fieldId: string, fieldType: string, error: string) => {
    trackEvent({
      type: 'field_error',
      fieldId,
      fieldType,
  ...
  const trackFieldValue = useCallback((fieldId: string, fieldType: string, value: string) => {
  ...
  const trackFormSubmit = useCallback((formData: Record<string, unknown>) => {
    ...
    trackEvent({
      ...
      eventData: {
        ...
        completedFields: Object.keys(formData).filter(key => formData[key] !== undefined && formData[key] !== '').length
      }
    });
  }, [trackEvent]);
