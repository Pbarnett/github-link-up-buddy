...
export const useTripOffers = ({
  tripId,
  initialTripDetails,
}: UseTripOffersProps): UseTripOffersReturn => {
  const [offers, setOffers] = useState<Offer[]>([]);
  const [tripDetailsState, setTripDetailsState] = useState<TripDetails | null>(
    initialTripDetails || null
  );
  ...
  const [usedRelaxedCriteriaState, setUsedRelaxedCriteriaState] =
    useState(false);
  const [lastRefreshTime, setLastRefreshTime] = useState<number>(0);

  const validateOfferDuration = useCallback(
    (offer: Offer, minDuration: number, maxDuration: number): boolean => {
      ...
      if (isNaN(departDate.getTime()) || isNaN(returnDate.getTime())) {
        logger.warn('Invalid date encountered in offer:', {
          offerId: offer.id,
  ...
  const loadOffers = useCallback(
    async (
      overrideFilterArg = true,
      relaxCriteriaArg = false,
      useCache = true
    ) => {
      logger.info('[useTripOffersLegacy] Loading offers', {
        tripId,
        overrideFilterArg,
        relaxCriteriaArg,
        useCache,
      ...
      if (!tripId) {
        logger.error(
          '[useTripOffersLegacy] No trip ID provided for loadOffers call.'
        );
        setHasError(true);
      ...
      if (useCache && !isRefreshing) {
        // currentCacheKey will always be defined here
        ...
        if (
          ...
          logger.info(
            '[useTripOffersLegacy] Using cached results from unifiedCache for key:',
            currentCacheKey
          );
      ...
      try {
        ...
        if (initialTripDetails && initialTripDetails.id === tripId) {
          ...
          logger.debug(
            '[useTripOffersLegacy] Fetching trip details from DB for tripId:',
            tripId
          );
          const { data: fetchedDbTripRequest, error: tripError } =
            await supabase
              .from('trip_requests')
              .select('*')
              .eq('id', tripId)
          ...
          currentTripDetails =
            mapTripRequestToTripDetails(fetchedDbTripRequest);
        ...
        logger.info(
          '[useTripOffersLegacy] calling invokeFlightSearch with payload:',
          flightSearchPayload
        ...
        try {
          ...
          logger.error(
            '[useTripOffersLegacy] Initial flight search failed, checking for existing offers:',
            { tripId, errorDetails: searchError }
          );

          const existingOffersRaw = await fetchTripOffers(tripId);
          if (existingOffersRaw.length > 0) {
            logger.info(
              `[useTripOffersLegacy] Found ${existingOffersRaw.length} existing offers, attempting to use as fallback.`
            );
            ...
            if (!overrideFilterArg && currentTripDetails) {
              finalExistingOffers = existingOffersRaw.filter(offer =>
                validateOfferDuration(
                  offer,
                  currentTripDetails.min_duration,
                  currentTripDetails.max_duration
                )
              );
            ...
            if (finalExistingOffers.length > 0) {
              ...
              const existingEntry = unifiedCache.get(currentCacheKey) || {
                timestamp: Date.now(),
              };
              ...
              logger.info(
                '[useTripOffersLegacy] Fallback results cached in unifiedCache for key:',
                currentCacheKey
              );
              setIsLoading(false);
              setHasError(false);
        ...
        if (!searchServiceResponse.success) {
          throw new Error(
            searchServiceResponse.message ||
              'Flight search invocation reported failure.'
          );
        }

        logger.debug(
          '[useTripOffersLegacy] Flight search invoked successfully.',
          { tripId, responseData: searchServiceResponse }
        );
        ...
        if (relaxCriteriaArg) {
          toast({
            ...
            description:
              'Finding flights with more flexible duration and budget constraints.',
          });
        }

        const fetchedOffers: Offer[] = await fetchTripOffers(tripId);
        logger.info(
          `[useTripOffersLegacy] Fetched ${fetchedOffers.length} offers via service for tripId: ${tripId}`
        );

        // Log summary of fetched offers
        logger.debug(
          `[useTripOffersLegacy] Fetched ${fetchedOffers.length} offers`,
          {
            tripId,
        ...
        if (fetchedOffers.length === 0) {
          logger.warn(
            '[useTripOffersLegacy] No offers found via service for tripId:',
            tripId
          );
          toast({
          ...
          if (currentTripDetails) {
            // Ensure currentTripDetails exists before caching
            const existingEntry = unifiedCache.get(currentCacheKey) || {
              timestamp: Date.now(),
            };
            unifiedCache.set(currentCacheKey, {
            ...
            logger.info(
              '[useTripOffersLegacy] Empty offerset cached in unifiedCache for key:',
              currentCacheKey
            );
          }
        } else {
          ...
          if (!overrideFilterArg && currentTripDetails) {
            logger.debug(
              `[useTripOffersLegacy] Applying duration filter: ${currentTripDetails.min_duration}-${currentTripDetails.max_duration} days`
            );

            const validOffers = fetchedOffers.filter(offer => {
              return validateOfferDuration(
                offer,
                currentTripDetails.min_duration,
                currentTripDetails.max_duration
              );
            ...
            logger.debug(
              `[useTripOffersLegacy] Bypassing duration filter (override=${overrideFilterArg}). Using all ${fetchedOffers.length} offers.`
            );
          ...
          if (currentTripDetails) {
            ...
            logger.info(
              '[useTripOffersLegacy] Results cached in unifiedCache for key:',
              currentCacheKey
            );
          }
        }
        ...
        logger.error('[useTripOffersLegacy] Error in loadOffers:', {
          tripId,
          errorDetails: error,
        });
        setHasError(true);
        const displayMessage =
          error.message || 'Something went wrong loading offers';
  ...
  const refreshOffers = useCallback(async () => {
    ...
    logger.info(
      '[useTripOffersLegacy] unifiedCache entry deleted for key:',
      cacheKey,
      'before refresh.'
    );
  ...
  const handleOverrideSearch = useCallback(() => {
    toast({
      ...
      description:
        'Finding all available flights regardless of trip duration...',
    });
