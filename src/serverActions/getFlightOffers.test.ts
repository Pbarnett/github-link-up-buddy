...
describe('getFlightOffers server action', () => {
  // Create a properly structured mock that matches the Supabase client interface
  ...
  it('should fetch flight offers successfully from the table (cache miss)', async () => {
    ...
    const result = await getFlightOffers(mockTripRequestId, undefined, deps);
  ...
  it('should throw an error if fetching from table fails', async () => {
    ...
    await expect(getFlightOffers(mockTripRequestId, undefined, deps)).rejects.toThrow(
  ...
  it('should use cached data for subsequent calls within cache duration', async () => {
    ...
    const result1 = await getFlightOffers(mockTripRequestId, undefined, deps);
    expect(result1).toEqual(mockDbRows);

    // Second call - should use cache (same result)
    const result2 = await getFlightOffers(mockTripRequestId, undefined, deps);
  ...
  describe('with refresh = true', () => {
    it('should invoke flight-search-v2, invalidate cache, then fetch from table', async () => {
      ...
      const mockInvokeEdgeFn = vi.fn().mockResolvedValue({
        data: { inserted: 1, message: 'Refreshed' },
        error: null,
      });
      const deps: GetFlightOffersDeps = {
        supabaseClient: mockSupabaseClient,
      ...
      const refreshedResult = await getFlightOffers(
        { tripRequestId: mockTripRequestId, refresh: true },
        undefined,
        deps
      );
    ...
    it('should throw error if flight-search-v2 invocation fails during refresh', async () => {
      ...
      const mockInvokeEdgeFn = vi.fn().mockResolvedValue({
        data: null,
        error: { message: refreshErrorMessage },
      });
      const deps: GetFlightOffersDeps = {
        supabaseClient: mockSupabaseClient,
        invokeEdgeFn: mockInvokeEdgeFn,
    ...
    it('refresh=true should still fetch from table if flight-search-v2 succeeds but finds no new offers (e.g. inserted: 0)', async () => {
      const mockInvokeEdgeFn = vi.fn().mockResolvedValue({
        data: { inserted: 0, message: 'No new offers' },
        error: null,
      });
      const deps: GetFlightOffersDeps = {
        supabaseClient: mockSupabaseClient,
  ...
  it('should use different cache entries for different tripRequestIds (table fetch)', async () => {
    ...
    const mockDbRows2: FlightOfferV2DbRow[] = [
      { ...mockDbRows[0], id: 'offer-3', trip_request_id: mockTripRequestId2 },
    ];
      ...
      if (tableName === 'trip_requests') {
        ...
        return {
          select: vi.fn().mockReturnValue({
            eq: vi
              .fn()
              .mockImplementation((field: string, tripRequestId: string) => {
                const dataToReturn =
                  tripRequestId === mockTripRequestId2
                    ? mockDbRows2
                    : mockDbRows;
                return {
                  not: vi
    ...
    const result2 = await getFlightOffers(mockTripRequestId2, undefined, deps);
    ...
    const result1Again = await getFlightOffers(mockTripRequestId, undefined, deps);
      ...
      if (tableName === 'trip_requests') {
        ...
        return {
          select: vi.fn().mockReturnValue({
            eq: vi.fn().mockReturnValue({
              not: vi.fn().mockResolvedValue({
                data: [{ ...mockDbRows[0], id: 'offer-new' }],
                error: null,
              }),
            }),
          }),
