          ...
          <CardContent>
            <Alert variant="destructive">
              ...
              <AlertDescription>
                ...
                <button onClick={refreshPools} className="ml-2 mt-2 p-1 border rounded bg-gray-100 hover:bg-gray-200 text-sm">Retry</button>
              </AlertDescription>
            </Alert>
          </CardContent>
        </Card>
    ...
    return (
      <div className="container mx-auto p-4">
        <Card>
          ...
          <CardContent>
            <EmptyStateCard />
            <button onClick={refreshPools} className="mt-4 p-2 border rounded bg-blue-500 text-white hover:bg-blue-600">Refresh Offers</button>
          </CardContent>
        </Card>
      </div>
    );
            ...
            <div className="flex items-center gap-2">
              {filterState.activeFiltersCount > 0 && (
                <Badge variant="secondary" className="text-sm">
                  {filterState.activeFiltersCount} filter{filterState.activeFiltersCount !== 1 ? 's' : ''} active
                </Badge>
              )}
              <Button
                variant="outline"
                    ...
                    <div className="flex-1 space-y-1">
                      ...
                      <div className="flex items-center gap-6 text-sm">
                        <div>
                          ...
                          <span className="ml-1 font-medium text-gray-900">{offer.airline}</span>
                        </div>
                        <div>
                          <span className="text-gray-500">Flight ID:</span>
                          <span className="ml-1 font-medium text-gray-900">{offer.id.slice(0, 8)}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4 ml-6">
                      <div className="text-right">
                        <div className="text-xl font-bold text-gray-900">
                          {formatCurrency(offer.price, filterState.options.currency || 'USD')}
                        </div>
                      </div>
                      <Button 
                        onClick={() => handleBookOffer(offer as unknown as Record<string, unknown>)}
                        size="sm"
        ...
        <Card className="bg-gray-50">
          ...
          <CardContent className="text-xs text-gray-600">
            ...
            <div className="mt-2">
              ...
              Budget: {filterState.options.budget ? formatCurrency(filterState.options.budget) : 'None'} • 
