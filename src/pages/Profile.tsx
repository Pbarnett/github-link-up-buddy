...
import AuthGuard from '@/components/AuthGuard'; } from '@/components/ProfileForm';

import {  } from '@/components/profile/SimpleProfileStatus';
import {  } from '@/hooks/use-toast';
import {  } from '@/hooks/useFeatureFlag';
@/components/profile/ProfileV2';
...
function convertToIndicatorData(
  ...
  const fields: ProfileField[] = Object.entries(fieldMappings).map(
    ([fieldId, config]) => {
      const value = profile?.[fieldId as keyof TravelerProfile];
      const isCompleted =
        fieldId === 'phone_verified' || fieldId === 'is_verified'
          ? Boolean(value)
          : Boolean(value && value !== '');
  ...
  return {
    ...
    lastUpdated: profile?.updated_at
      ...
      : new Date(),
    tier:
      completenessScore.overall >= 100
        ? 'verified'
        : completenessScore.overall >= 70
          ? 'complete'
          : 'basic',
  };
}
...
function EnhancedProfilePage() {
  ...
  const { data: showWalletUI, isLoading: walletFlagLoading } =
    useFeatureFlag('wallet_ui');
    ...
    switch (action) {
      ...
      case 'verify_phone':
        ...
        toast({
          ...
          description:
            'Update your phone number in the form below, then verify it.',
        });
        break;
      case 'add_phone':
        setActiveTab('profile');
        toast({
          ...
          description:
            'Add your phone number in the form below for SMS notifications.',
        });
        break;
      case 'add_passport':
        setActiveTab('profile');
        toast({
          ...
          description:
            'Add your passport details for international travel bookings.',
        });
        break;
      default:
        toast({
    ...
    <div className="min-h-screen bg-gray-50" data-testid="profile-page">
      <div className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
        <div className="space-y-6">
          ...
          >
            <TabsList
              className={`grid w-full ${showWalletUI && !walletFlagLoading ? 'grid-cols-4' : 'grid-cols-3'}`}
            >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        ...
                        <p className="text-sm text-muted-foreground">
                          Enhanced profile completeness tracking with detailed
                          recommendations.
                        </p>
                      </div>
                      <div className="space-y-2">
                        ...
                        <p className="text-sm text-muted-foreground">
                          Intelligent notification preferences based on your
                          travel patterns.
                        </p>
                      </div>
                    </div>
...
function ProfilePage() {
  const { data: showEnhancedProfile, isLoading: flagLoading } =
    useFeatureFlag('profile_ui_revamp');
