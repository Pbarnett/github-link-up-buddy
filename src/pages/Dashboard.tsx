...
import TripHistory from '@/components/dashboard/TripHistory'; // Added import } from '@/components/personalization/GreetingBanner';
...
const Dashboard = () => {
  ...
  const loadDashboardData = async (userId: string) => {
    await Promise.all([loadBookingRequests(userId), loadTripRequests(userId)]);
  };
  ...
  const retryBookingRequest = async (requestId: string) => {
    try {
      const query = supabase.from('booking_requests');
  ...
  const getStatusIcon = (status: string) => {
    ...
    switch (status) {
      case 'done':
        return (
          <CheckCircle
            className={`${iconClasses} text-green-500 drop-shadow-sm`}
          />
        );
      case 'failed':
        return (
          <XCircle className={`${iconClasses} text-red-500 drop-shadow-sm`} />
        );
      case 'processing':
        return (
          <RefreshCw
            className={`${iconClasses} text-blue-500 animate-spin drop-shadow-sm`}
          />
        );
      case 'pending_booking':
      case 'pending_payment':
        return (
          <Clock className={`${iconClasses} text-amber-500 drop-shadow-sm`} />
        );
      default:
        return (
          <AlertCircle
            className={`${iconClasses} text-gray-500 drop-shadow-sm`}
          />
        );
    }
  };
  ...
  const getStatusCounts = () => {
    return {
      ...
      pending_booking: bookingRequests.filter(
        r => r.status === 'pending_booking'
      ).length,
      pending_payment: bookingRequests.filter(
  ...
  const renderPrice = (offerData: Record<string, unknown> | null) => {
    ...
    return (
      <span className="flex items-center">
        <DollarSign className="h-3 w-3 mr-1" />${String(price)}
      </span>
    );
  };
        ...
        <div className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            {[1, 2].map(i => (
              <div
                key={i}
                className="card-enhanced p-6 overflow-hidden relative"
              >
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="space-y-4">
                {[1, 2, 3].map(i => (
                  <div
                    key={i}
                    className="border border-gray-100 rounded-xl p-4 relative overflow-hidden"
                  >
                    <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/60 to-transparent -translate-x-full animate-shimmer transform"></div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
        ...
        0 ? (
            ...
            <CardContent className="space-y-6">
              <div className="space-y-3">
                ...
                <p className="text-lg text-gray-600 max-w-2xl mx-auto">
                  Tell us your perfect flight and price; we'll book it the
                  moment it appears.
                </p>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            ...
            <Card
              className="card-enhanced border-green-200 hover:border-green-300 cursor-pointer group"
              onClick={() =>
                (
                  document.querySelector('[value="tripHistory"]') as HTMLElement
                )?.click()
              }
            >
            ...
            <Card
              className="card-enhanced border-blue-200 hover:border-blue-300 cursor-pointer group"
              onClick={() =>
                (
              ...
              <Card className="card-enhanced mt-4">
                <CardHeader className="pb-4">
                  ...
                  <CardDescription className="text-gray-600">
                    Monitor your active flight searches and auto-booking
                    requests
                  </CardDescription>
                </CardHeader>
                <CardContent>
                                    ...
                                    <div className="flex-1 min-w-0">
                                      <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-2">
                                        <p className="font-medium text-gray-900 truncate">
                                          {request.offer_data &&
                                          typeof request.offer_data ===
                                            'object' &&
                                          'airline' in request.offer_data
                                            ? String(request.offer_data.airline)
                                            : ''}{' '}
                                          {request.offer_data &&
                                          typeof request.offer_data ===
                                            'object' &&
                                          'flight_number' in request.offer_data
                                            ? String(
                                                request.offer_data.flight_number
                                              )
                                            : ''}
                                        </p>
                                        <Badge
                                    ...
                                    {request.status === 'failed' &&
                                      request.attempts < 3 && (
                                        <Button
                                          ...
                                          variant="outline"
                                          onClick={() =>
                                            retryBookingRequest(request.id)
                                          }
                                          className="text-xs"
                                        >
            ...
            <TabsContent value="tripHistory">
              <Card className="mt-4">
                <CardHeader>
                  ...
                  <CardDescription>
                    Review your completed flight bookings.
                  </CardDescription>
                </CardHeader>
                <CardContent>
