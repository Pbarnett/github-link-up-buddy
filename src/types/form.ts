...
export const tripFormSchema = z
  .object({
    // Core date fields with better validation
    earliestDeparture: z
      .date({
        required_error: 'Earliest departure date is required',
        invalid_type_error: 'Please provide a valid departure date',
      })
    ...
    latestDeparture: z
      .date({
        ...
        invalid_type_error: 'Please provide a valid return search end date',
      })
      .refine(date => date > new Date(), {
        message: 'Latest departure date must be in the future',
      }),

    // Duration fields with improved validation
    min_duration: z.coerce
      ...
      .max(30, 'Minimum duration cannot exceed 30 days'),
    max_duration: z.coerce
      ...
      .max(30, 'Maximum duration cannot exceed 30 days'),

    // Price field - conditional validation based on auto_book_enabled
    max_price: z.coerce
      .number({
        invalid_type_error: 'Price must be a valid number',
      })
      .min(100, 'Maximum price must be at least $100')
      .max(10000, 'Maximum price cannot exceed $10,000')
      .optional(),

    // Airport selection fields
    nyc_airports: z.array(z.string()).default([]),
  ...
  .superRefine((data, ctx) => {
    ...
    // 6. Auto-booking validation with improved error handling
    if (data.auto_book_enabled) {
      // Validate max_price when auto-booking is enabled
      ...
      // Validate payment method when auto-booking is enabled
      ...
      // Auto-booking consent validation - explicit check for false
      if (data.auto_book_consent !== true) {
        ctx.addIssue({
