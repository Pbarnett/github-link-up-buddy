...
const IdentityVerification: FC<IdentityVerificationProps> = ({
    ...
    try {
      ...
      setError(
        err instanceof Error
          ? err.message
          : 'Failed to check verification requirement'
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const getStatusDisplay = () => {
    ...
    return (
      <div
        className={`px-3 py-2 rounded-md text-sm font-medium ${colors[status]}`}
      >
        {messages[status]}
      </div>
    );
    ...
    <div className={`space-y-4 ${className}`}>
        ...
        <div className="flex items-start space-x-3">
          ...
          <div className="flex-1">
            ...
            <p className="mt-1 text-sm text-gray-600">
              {purpose === 'identity_document' &&
                'We need to verify your identity document for this booking.'}
              {purpose === 'address' &&
                'We need to verify your address for this booking.'}
                  ...
                  <div className="text-sm text-green-600">
                    ...
                    {verificationSession.verified_outputs.document && (
                      <div className="mt-2 text-xs">
                        Document:{' '}
                        {verificationSession.verified_outputs.document.type}{' '}
                        from{' '}
                        {
                          verificationSession.verified_outputs.document
                            .issuing_country
      ...
      <div className="bg-gray-50 p-4 rounded-md">
        ...
        <ul className="text-xs text-gray-600 space-y-1">
          <li>
            • Government-issued photo ID (passport, driver's license, or
            national ID)
          </li>
        ...
        <p className="text-xs text-gray-500 mt-2">
          Your information is securely processed by Stripe and deleted after
          verification.
        </p>
      </div>
    </div>
