...
const PoolOfferControls: FC<PoolOfferControlsProps> = ({ tripId }) => {
  ...
  const { budget, maxBudget, dateRange, bumpsUsed, bumpBudget } =
  ...
  const toggleNonStop = () => {
    ...
    toast({
      title: nonStopOnly
        ? 'Direct flights only disabled'
        : 'Direct flights only enabled',
      description: nonStopOnly
        ? 'Showing all flight options'
        : 'Showing only non-stop flights',
    });
  };
  ...
  return (
    <div
      className="space-y-4 p-4 bg-white rounded-lg border ring-focus"
      tabIndex={-1}
    >
      <ConstraintChips
        dateRange={dateRange}
