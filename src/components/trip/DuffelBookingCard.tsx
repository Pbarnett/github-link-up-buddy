...
} from '@/services/api/duffelBookingApi';
...
export const DuffelBookingCard: FC<DuffelBookingCardProps> = ({
  ...
  const [bookingResult, setBookingResult] =
    useState<DuffelBookingResponse | null>(null);
  ...
  const handleBookFlight = async () => {
    ...
    try {
      console.log(
        '[DUFFEL-BOOKING-CARD] Starting booking process for offer:',
        offer.id
      );
      ...
      if (result.success && result.order) {
        ...
        const errorMessage =
          result.error?.message || 'Booking failed. Please try again.';
      ...
      <Card className="w-full max-w-2xl mx-auto">
        ...
        <CardContent className="space-y-4">
          <Alert>
            ...
            <AlertDescription>
              Your flight has been successfully booked. You should receive a
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <p className="text-lg font-mono">
                {bookingResult.order.booking_reference}
              </p>
            </div>
            <div>
              ...
              <p className="text-lg">
                {formatPrice(
                  parseFloat(bookingResult.order.total_amount),
                  bookingResult.order.total_currency
                )}
              </p>
            </div>
          </div>

          {bookingResult.order.tickets &&
            bookingResult.order.tickets.length > 0 && (
              <div>
        ...
        <div className="border rounded-lg p-4 bg-gray-50">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-lg font-semibold">
              {getAirportName(offer.origin_airport)} →{' '}
              {getAirportName(offer.destination_airport)}
            </h3>
        ...
        <div className="border rounded-lg p-4 bg-gray-50">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <p className="font-medium">
                {traveler.title} {traveler.given_name} {traveler.family_name}
              </p>
            </div>
            <div>
