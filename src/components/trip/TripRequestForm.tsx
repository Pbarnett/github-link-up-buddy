...
import logger from '@/lib/logger'; } from '@/services/api/flightSearchApi';
...
import AutoBookingSection from './sections/AutoBookingSection'; } from 'react';
...
const LegacyTripRequestForm: React.FC<TripRequestFormProps> = ({
  tripRequestId,
  mode = 'manual',
    ...
    if (tripRequestId) {
      const fetchTripDetails = async () => {
        ...
        try {
          ...
          const tripData = await repository.findById(tripRequestId, {
            throwOnEmpty: true,
          });

          if (tripData) {
            const { nycAirports, otherAirport } = categorizeAirports(
              tripData.departure_airports
            );
            reset({
              earliestDeparture: tripData.earliest_departure
                ? parseISO(tripData.earliest_departure)
                : undefined,
              latestDeparture: tripData.latest_departure
                ? parseISO(tripData.latest_departure)
                : undefined,
              ...
              destination_airport:
                tripData.destination_airport?.length === 3 &&
                tripData.destination_airport ===
                  tripData.destination_airport?.toUpperCase()
                  ? tripData.destination_airport
                  : '',
              destination_other:
                tripData.destination_airport?.length !== 3 ||
              ...
              baggage_included_required:
                tripData.baggage_included_required ?? false,
  ...
  const validateFormData = (data: FormValues): boolean => {
    const destinationAirport =
      data.destination_airport || data.destination_other || '';
  ...
  const transformFormData = (data: FormValues): ExtendedTripFormValues => {
    ...
    return {
      ...
      preferred_payment_method_id: isAutoBookingEnabled
        ? data.preferred_payment_method_id
        : null,
    };
  };
  ...
  const createTripRequest = async (
    ...
    if (!userId)
      throw new ValidationError(
        'You must be logged in to create a trip request.'
      );
  ...
  const updateTripRequest = async (
    formData: ExtendedTripFormValues
  ): Promise<TripRequestFromDB> => {
    if (!userId || !tripRequestId) {
      throw new ValidationError(
        'User ID or Trip Request ID is missing for update.'
      );
    }
  ...
  const navigateToConfirmation = (tripRequest: TripRequestFromDB): void => {
    ...
    const autoBookText = tripRequest.auto_book_enabled
      ? tripRequestId
        ? ' Auto-booking settings updated.'
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      console.log(
        'DEBUG: formData.destination_airport:',
      ...
      console.log(
        'DEBUG: transformedData.destination_location_code:',
        transformedData.destination_location_code
      );
      ...
      if (tripRequestId) {
        ...
        try {
          ...
          const timeDiff =
            transformedData.latestDeparture.getTime() -
            transformedData.earliestDeparture.getTime();
          ...
          console.warn(
            'Flight search failed during form submission:',
            searchError
          );
          toast({
      ...
      logger.error(
        `Error ${tripRequestId ? 'updating' : 'creating'} trip request:`,
        {
          tripRequestId,
  ...
  const isStep1Valid = Boolean(
    watchedFields.earliestDeparture &&
      ...
      ((watchedFields.nyc_airports && watchedFields.nyc_airports.length > 0) ||
  ...
  return isLoadingDetails ? (
    ...
    <div className="min-h-screen bg-gray-50 p-4">
      <div className="container mx-auto py-8 space-y-6">
        ...
        <div
          className={`grid gap-6 ${mode === 'auto' && !isMobile ? 'lg:grid-cols-3' : 'grid-cols-1'}`}
        >
          {/* Main Form Column */}
          <div
            className={`${mode === 'auto' && !isMobile ? 'lg:col-span-2' : 'col-span-1'}`}
          >
                    ...
                    <TripSummaryChips
                      ...
                      onClearField={fieldName => {
                        form.setValue(
                          ...
                          fieldName === 'nyc_airports' ? [] : undefined
                        );
                      }}
                    />
...
const TripRequestForm = ({
  ...
  return <LegacyTripRequestForm tripRequestId={tripRequestId} mode={mode} />;
};
