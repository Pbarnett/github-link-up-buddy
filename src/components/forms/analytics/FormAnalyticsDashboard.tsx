...
export const FormAnalyticsDashboard: React.FC = () => {
  ...
  const getCompletionRateBadge = (rate: number): 'default' | 'secondary' | 'destructive' => {
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold">{overview.totalSubmissions.toLocaleString()}</p>
                </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className={`text-2xl font-bold ${getCompletionRateColor(overview.avgCompletionRate)}`}>
              ...
              <Card className="col-span-2">
                <CardContent className="p-6 text-center">
                  ...
                  <p className="text-muted-foreground">
                    Start using the dynamic forms system to see analytics data here.
                  </p>
                </CardContent>
              </Card>
            ) : (
                ...
                <Card key={form.form_name}>
                  <CardHeader>
                    <div className="flex items-center justify-between">
                      ...
                      <Badge variant={getCompletionRateBadge(form.completion_rate)}>
                  ...
                  <CardContent className="space-y-4">
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-medium">{form.total_views.toLocaleString()}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <CheckCircle className="h-4 w-4 text-muted-foreground" />
                        <span className="text-muted-foreground">Submissions:</span>
                        <span className="font-medium">{form.total_submissions.toLocaleString()}</span>
                      </div>
                    </div>
                    ...
                    {form.avg_completion_time_ms > 0 && (
                      <div className="flex items-center gap-2 text-sm">
                        ...
                        <span className="font-medium">{formatDuration(form.avg_completion_time_ms)}</span>
                      </div>
                    )}
                  </CardContent>
                </Card>
            ...
            <CardContent>
              {analytics.length === 0 ? (
                <div className="text-center py-8">
                  <p className="text-muted-foreground">No form data available for the selected period.</p>
                </div>
              ) : (
                    ...
                    <div key={form.form_name} className="border rounded-lg p-4">
                      ...
                      <div className="w-full bg-gray-200 rounded-full h-2 mb-2">
                        <div 
                          ...
                          style={{ width: `${Math.min(form.completion_rate, 100)}%` }}
                        ></div>
                      </div>
        ...
        <TabsContent value="insights" className="space-y-4">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card>
              ...
              <CardContent className="space-y-4">
                {analytics.length === 0 ? (
                  <p className="text-muted-foreground">No insights available yet. Start collecting form data to see recommendations.</p>
                ) : (
                  <div className="space-y-3">
                    {analytics.some(form => form.completion_rate < 50) && (
                      <Alert>
                        <AlertTriangle className="h-4 w-4" />
                        <AlertDescription>
                          Some forms have low completion rates (under 50%). Consider simplifying these forms or reducing the number of required fields.
                        </AlertDescription>
                      </Alert>
                    )}
                    ...
                    {analytics.some(form => form.completion_rate > 80) && (
                      <Alert>
                        ...
                        <AlertDescription>
                          Great job! Some forms have high completion rates (over 80%). Consider using their structure as a template for other forms.
                        </AlertDescription>
                      </Alert>
                    )}
                    ...
                    <Alert>
                      ...
                      <AlertDescription>
                        Monitor completion rates regularly. Industry average for web forms is around 68%.
                      </AlertDescription>
                    </Alert>
                  </div>
                )}
