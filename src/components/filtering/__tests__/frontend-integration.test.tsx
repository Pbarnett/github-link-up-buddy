...
// Mock AirlineSelector component
vi.mock('../AirlineSelector', () => {
  return {
    default: ({ selectedAirlines, onSelectionChange }: any) => (
      <div data-testid="airline-selector">
        <span>Airlines: {selectedAirlines?.length || 0} selected</span>
        <button
          onClick={() => onSelectionChange(['AA', 'DL'])}
          data-testid="select-airlines"
        >
          Select Airlines
        </button>
      </div>
