  ...
  return isLoadingDetails ? (
    ...
    <div className="min-h-screen bg-gray-50 p-4">
      <div className="container mx-auto py-8 space-y-6">
    	...
    	{/* SummaryBar appears only with the UI refresh flag and mirrors current form values */}
    	{uiRefresh && (
    	  <SummaryBar
    	    value={{
    	      from: (() => {
    	        const origins: string[] = [] as any;
    	        const nyc = (form.watch('nyc_airports' as any) as any) || [];
    	        const other = form.watch('other_departure_airport' as any) as any;
    	        if (Array.isArray(nyc) && nyc.length) origins.push(...nyc);
    	        if (other) origins.push(other);
    	        return origins;
    	      })(),
    	      to: (() => {
    	        const iata = form.watch('destination_airport' as any) as any;
    	        const other = form.watch('destination_other' as any) as any;
    	        const d = iata || other;
    	        return d ? [d] : [];
    	      })(),
    	      nights: [form.watch('min_duration' as any) as any, form.watch('max_duration' as any) as any],
    	      depart: [form.watch('earliestDeparture' as any) as any, form.watch('latestDeparture' as any) as any],
    	      price: form.watch('max_price' as any) as any,
    	      nonstop: form.watch('nonstop_required' as any) as any,
    	    }}
    	    cta={buttonText()}
    	    onSubmit={form.handleSubmit(handleStepSubmit)}
    	  />
