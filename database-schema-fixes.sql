-- ===============================================
-- PRODUCTION READINESS: DATABASE SCHEMA FIXES
-- ===============================================
-- Run this SQL in Supabase SQL Editor to fix all database issues
...
-- 3. Create missing queue tables if they don't exist
...
-- 4. Create events table for event sourcing
...
-- 5. Create notification_deliveries table for tracking
...
CREATE INDEX IF NOT EXISTS idx_queue_scheduled ON public.notification_queue(scheduled_for) WHERE processed_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_critical_queue_scheduled ON public.critical_notification_queue(scheduled_for) WHERE processed_at IS NULL;
...
('booking_failure_email', 'booking_failure', 'email', '⚠️ Flight Booking Issue',
 'We encountered an issue with your flight booking attempt. Please contact support or try again.',
 '<h1>Booking Issue</h1><p>We encountered an issue with your flight booking attempt.</p><p><b>Details:</b> {{error}}</p><p>Please contact support or try again.</p>'),

('booking_failure_sms', 'booking_failure', 'sms', null,
 'Booking failed: {{error}}. Please try again or contact support.', null)
...
-- 10. Create RLS policies for service role access
CREATE POLICY IF NOT EXISTS "Service role can manage all templates" ON public.notification_templates
  FOR ALL USING (auth.role() = 'service_role');

CREATE POLICY IF NOT EXISTS "Service role can manage all deliveries" ON public.notification_deliveries
  FOR ALL USING (auth.role() = 'service_role');

CREATE POLICY IF NOT EXISTS "Service role can manage all events" ON public.events
  FOR ALL USING (auth.role() = 'service_role');
...
-- ===============================================
-- All tables, columns, indexes, and permissions are now set up
